...
class RidgeRegression(BaseEstimator):
    ...
    def _fit(self, X: np.ndarray, y: np.ndarray) -> NoReturn:
        ...
        """
        lamID = self.lam_ * np.identity(X.shape[1])
        if self.include_intercept_:
            X = np.insert(X,0,[np.ones(X.shape[0])],axis= 1)
            lamID = np.insert(lamID,0,[np.zeros(lamID.shape[1])],axis= 0)
            lamID = np.insert(lamID,0,[np.zeros(lamID.shape[0])],axis= 1)
        self.coefs_ = inv((X.T @ X) + lamID) @ X.T @ y
